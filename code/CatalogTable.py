...
class CatalogTable:
    ...
    def apply_cut(self, cut):
        ...
        colors =  {"G": """self.table["phot_g_mean_mag"]""",
                   "H": """self.table["h_m"]""",
                   "J": """self.table["j_m"]""",
                   "K": """self.table["k_m"]""",
                   "W1": """self.table["w1mpro"]""",
                   "W2": """self.table["w2mpro"]""",
                   "W3": """self.table["w3mpro"]""",
                   "W4": """self.table["w4mpro"]""",
                   "BP-RP": """self.table["bp_rp"]"""}
        ...
        cut_true = self.table[pd.eval(cut)]
        cut_false = self.table[~pd.eval(cut)]
    ...
    def variability_cut(self):
        ...
        cut_table = self.table.copy()

        mask_shape = []

        variability = [np.sqrt( g_n_obs ) / mean_flux_over_error for g_n_obs, mean_flux_over_error in zip(self.table["phot_g_n_obs"], self.table["phot_g_mean_flux_over_error"])]

        for g, v in zip(self.table["phot_g_mean_mag"], variability):
    ...
    def gaia_color_mag_cut(self):
        ...
        M_g = [g + 5 - np.log10(1000/p) -10 for g,p in zip(self.table["phot_g_mean_mag"], self.table["parallax"])]

        mask_shape = []

        for col, m_g in zip(self.table["bp_rp"], M_g):
    ...
    def subtract_cols(self, colname1, colname2):
        ...
        return([i-j for i,j in zip(self.table[colname1], self.table[colname2])])

    def plot_images(self, table, survey_list, img_list, cats_to_plot=None):
